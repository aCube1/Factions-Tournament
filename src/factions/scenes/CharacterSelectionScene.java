...
public class CharacterSelectionScene implements IScene {
    // I HATE Java and C++ for not having proper enums
    ...
    private void updatePreviewPanel(Panel preview_panel, int index) {
        preview_panel.removeAllComponents();

        Panel new_preview = createPreviewPanel(index);

        for (Component component : new_preview.getChildren()) {
            preview_panel.addComponent(component);
        }
        preview_panel.invalidate();
    }

    private Panel createHorizontalSelector(AtomicInteger current_selection, Panel preview_panel) {
        Panel selector = new Panel();
        selector.setLayoutManager(new GridLayout(CHARACTER_NAMES.length));

        // I don't know what happens here, but I will trust the IA this once
        for (int i = 0; i < CHARACTER_NAMES.length; i++) {
            final int index = i;
            Panel char_panel = createSelectableCharacterCard(i, current_selection.get() == i);

            Button char_button = new Button(CHARACTER_NAMES[i], new Runnable() {
                @Override
                public void run() {
                    current_selection.set(index);
                    updatePreviewPanel(preview_panel, index);
                }
            });

            char_button.addListener((component) -> {
                current_selection.set(index);
                updatePreviewPanel(preview_panel, index);
            });
    ...
    private Panel createPreviewPanel(int idx) {
        ...
        Panel left_panel = new Panel(); // For Large Sprite + Name
    ...
    private Panel createSelectionPanel() {
        ...
        center_panel.setLayoutManager(new LinearLayout(Direction.VERTICAL));
        ...
        center_panel.addComponent(title_panel);
        center_panel.addComponent(new EmptySpace(new TerminalSize(0, 1)));

        // It's kinda weird that Lanterna render on multiple threads
        AtomicInteger current_selection = new AtomicInteger(0);

        Panel preview_panel = createPreviewPanel(0);
        center_panel.addComponent(preview_panel
                .withBorder(Borders.doubleLine("Character Info")));

        center_panel.addComponent(new EmptySpace(new TerminalSize(0, 1)));

        Panel selector_panel = createHorizontalSelector(current_selection, preview_panel);
        center_panel.addComponent(selector_panel
        ...
        Button select_btn = new Button("[O] Confirm Selection", () -> {
            _selected_character = current_selection.get();
        ...
        center_panel.addComponent(button_panel);
